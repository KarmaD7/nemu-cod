...
static word_t vaddr_mmu_read(struct Decode *s, vaddr_t addr, int len, int type) {
  ...
  if (ret == MEM_RET_OK) {
    ...
    if (unlikely(dynamic_config.debug_difftest)) {
      fprintf(stderr, "[NEMU] mmu_read: vaddr 0x%x, paddr 0x%x, rdata 0x%x\n",
        vaddr, addr, rdata);
    }
#endif
    return rdata;
...
#ifdef CONFIG_SHARE
    if (unlikely(dynamic_config.debug_difftest)) {
      fprintf(stderr, "[NEMU] mmu_write: vaddr 0x%x, paddr 0x%x, len %d, data 0x%x\n",
        vaddr, addr, len, data);
    }
#endif
